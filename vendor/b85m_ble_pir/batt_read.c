...
//batt configuration
#define GPIO_VBAT_DETECT				GPIO_PA1
...
u16 PIR_Get_Batt_V()
{
    ...
    volatile unsigned int adc_dat_buf[ADC_SAMPLE_NUM];  //size must 16  byte aligned(16/32/64...)
      ...
      adc_set_state_length(240, 0, 10);   //set R_max_mc,R_max_c,R_max_s//捕获长度240状态长度10状态周期10.4uS
...
//set misc channel use differential_mode,
      ...
      dfifo_enable_dfifo2();
//get adc sample data and sort these data  //获取adc样本数据并对这些数据进行排序
